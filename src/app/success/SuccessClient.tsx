...
//   );
// }
// src/app/success/SuccessClient.tsx (or wherever this lives)
'use client';
...
export default function SuccessClient() {
  ...
  // Load session data from localStorage that we saved on the form page
  useEffect(() => {
    if (!sessionId) return;
    if (typeof window === 'undefined') return;

    try {
      const raw = window.localStorage.getItem('elf-current-session');
      if (!raw) return;
      ...
      console.error('Error reading elf-current-session from localStorage', err);
    }
  }, [sessionId]);

  async function handleGenerate() {
    if (!sessionId) {
      setError('Missing session ID. Please go back to the homepage and try again.');
      return;
    }
    if (!sessionData) {
      setError(
        'I couldn’t find your Elf details. Please go back to the homepage and refill the form.',
      );
      return;
    }
